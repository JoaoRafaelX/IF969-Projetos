        ...
        while i < len(visitados):
            ...
            for vizinho in self[atual-1]:
                if not vizinho in visitados:
                    #print(str(atual), "\t", str(visitados[:i]), "[" + str(visitados[i]) + "]", str(visitados[i+1:]))
                    if vizinho == v:
                        #print(str(visitados + [vizinho]))
...
class Controle():
    ...
    def buscarLargura(self, u=1, v=-1):
        ...
        if v == 0:
            if self.grafo.direcional:
                return self.grafo.buscarLarguraPercorrer()
        return self.grafo.buscarLargura(u, v)
...
c.lerArquivo("olar.txt")
rel = Relogio("Percorrer")
print(c.grafo.buscarLarguraPercorrer())
del rel
rel = Relogio("Buscar 0")
print(c.grafo.buscarLargura(1,0))
del rel
